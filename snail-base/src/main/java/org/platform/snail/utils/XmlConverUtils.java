...
public class XmlConverUtils {
	...
	public static String getReportXML(List<Map<String,Object>> list,Map<String,Object> p) throws Exception {
		Document document = DocumentHelper.createDocument();
		Element report = document.addElement("report");
		Element newDataSet = report.addElement("NewDataSet");
		for (Object o : list) {
			Element nodeElement = newDataSet.addElement("Table");
			if (o instanceof Map) {
				for (Object obj : ((Map) o).keySet()) {
		...
		Element _grparam = report.addElement("_grparam");
		for (Object obj : p.keySet()) {
			Element param = _grparam.addElement(String.valueOf(obj));
			param.setText(String.valueOf((p.get(obj))));
		}
		return doc2String(document);
	}
