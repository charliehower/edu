			...
			.dialog(
					{
						...
						width : 300,
						height : 400,
						title : "<div class='widget-header widget-header-small'><h4 class='smaller'><i class='ace-icon fa fa-cog'></i>日程安排</h4></div>",
						title_html : true,
						buttons : [

								{
									...
									click : function() {
										if (oper == 'save') {
											...
											deleteScheduleByScheduleId(ev.id);
										}
...
function initEvents(userId) {
	// alert(userId);
	$.ajax({
		type : "post",
		url : contextPath + "/schedule/findScheduleList.do",
		data : {
			start : 0,
			limit : 99999,
			totalRecord : 9999,
...
function initDepUserList() {
	 ...
	 * "/schedule/selectDepUserListByDepId.do", data : { }, beforeSend :
...
function updateSchedule(dialog, id) {
	...
	// o.allDay = $('input[name=allDay]').val();
	if (o.title == '') {
		alert("标题不能为空！");
		return;
	}
	...
	$.ajax({
		type : "post",
		url : contextPath + "/schedule/updateSchedule.do",
		data : {
...
function insertSchedule(dialog) {
	...
	$.ajax({
		type : "post",
		url : contextPath + "/schedule/insertSchedule.do",
		data : {
			jsons : JSON.stringify(o)
		},
		beforeSend : function(XMLHttpRequest) {
...
function deleteScheduleByScheduleId(id) {

	$.ajax({
		type : "post",
		url : contextPath + "/schedule/deleteScheduleByScheduleId.do",
		data : {
			id : id
		},
		beforeSend : function(XMLHttpRequest) {
...
jQuery(function($) {
	...
	$('#tree-dept').ace_tree({
						dataSource : new DataSourceTree({
							url : contextPath
									+ '/users/selectGroupDepAceTreeByPid.do'
						}),
						multiSelect : false,
