...
function init_uploader(){
	 $("#uploader").pluploadQueue({
	        ...
	        chunk_size : '1mb',
	        unique_names : true,
	        multipart_params:{collectionName:'performance'}, 
	        filters : {
	            max_file_size : '10mb',
	            mime_types: [
{title : "Image files", extensions : "jpg,gif,png,pdf,bmp"},
{title : "Zip files", extensions : "rar,zip,doc,xls,xlsx"}
	    ...
	    uploader.bind("FileUploaded", function (uploader,file,responseObject) {
   			...
   			if (!rst.state) {
				...
				$('input[name=file]').val(rst.response);
				$( "#dialog-message" ).dialog( "close" );
			}
	    });
}
...
function initCompent(){
	...
	$( "#btn-upload-view" ).on('click', function(e) {
		...
		var dialog = $( "#dialog-message-file" ).removeClass('hide').dialog({
			modal: true,
			width:500,
			title: "<div class='widget-header widget-header-small'><h4 class='smaller'><i class='ace-icon fa fa-cog'></i> 文件</h4></div>",
			title_html: true,
			buttons: [ 
				{
					html: "<i class='ace-icon fa fa-check bigger-110'></i>&nbsp; 确定",
		...
		var src=portalContextPath+'/files/download.do?collectionName=performance&originalFilename='+fileName+'&fileName='+fileName;
			console.log(src);
			window.open(src);
		 var img = new Image();
		 $(img).attr("src", "");
