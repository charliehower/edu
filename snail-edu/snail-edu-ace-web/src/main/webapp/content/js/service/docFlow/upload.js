jQuery(function($) {
	...
	var uploader = new plupload.Uploader({
		...
		url : portalContextPath+'/attach/uploadFile.do?noticeId='+id+'&collectionName=docFlow',
		...
		init: {
			...
			FilesAdded: function(up, files) {
				plupload.each(files, function(file) {
					//alert(file.id);
					document.getElementById('filelist').innerHTML += '<div class="fileList0" id="' + file.id + '">' + file.name + ' (' + plupload.formatSize(file.size) + ') <b></b></div>';
				});
			},
